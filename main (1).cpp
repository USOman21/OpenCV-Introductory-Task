...
int main( )
{
    ...
    while(1)    // Main loop to perform image processing
    {
        ...
        Mat frame = imread("redblackline.png"); // Open an image file and store in a new matrix variable
        int pixelCount[3];
        int avg[3];
        ...
        for(int n = 0; n < 3; n++)
        {
            ...
            int lowHx[] = {lowH, lowS, lowV};
            int highHx[] = {highH, highS, highV};

            Mat frameHSV;       // Convert the frame to HSV and apply the limits
            cvtColor(frame, frameHSV, COLOR_BGR2HSV);
            inRange(frameHSV, Scalar(lowHx[n], lowS, lowV), Scalar(highHx[n], highS, highV), frameHSV);
            ...
            Mat frameGrey;
            cvtColor(frameHSV, frameGrey, CV_BGR2GRAY);
            printf("%d\n", countNonZero(frameGrey));
            avg[n] = countNonZero(frameGrey);
